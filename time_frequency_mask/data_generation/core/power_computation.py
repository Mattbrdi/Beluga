...
from time_frequency_mask.stft import (
    frequency_band,
    scipy_stft_complex_psd,
)


def compute_mean_power(
    waveform: NDArray[np.float64],
    parameters: Parameters,
    mask=None,
) -> float:
    freqs, time, Zxx = scipy_stft_complex_psd(
        waveform, parameters.audio.sampling_rate, parameters.stft
    )
    freqs, Zxx = frequency_band(
        freqs, Zxx, parameters.audio.min_freq, parameters.audio.max_freq
    )
    Pxx = np.abs(Zxx) ** 2
    ...
    df = parameters.audio.sampling_rate / parameters.stft.n_fft
...
def compute_P_in_t(
    ...
    *,
    ...
    if Pxx.shape != mask.shape:
        raise ValueError(
            f"Incorrect shape got {Pxx.shape} for Zxx and {mask.shape} for mask"
        )
...
def compute_noise_power_t(
    Pxx: NDArray[np.float64],
    mask: NDArray[np.uint8],
    fallback_noise_psd: float,
    *,
    df: float,
) -> NDArray[np.float64]:
    if Pxx.shape != mask.shape:
...
def compute_P_moy(
    Pxx_ij: NDArray[np.float64],
    mask_ij: NDArray[np.bool_],
    fallback_noise_psd: float,
    *,
    df: float,
) -> tuple[float, float]:
    ...
    P_signal_and_noise_in_bin_t = compute_P_in_t(Pxx_ij, mask_ij, df=df)
    P_noise_in_bin_t = compute_noise_power_t(Pxx_ij, mask_ij, fallback_noise_psd, df=df)

    P_signal_in_bin_t = P_signal_and_noise_in_bin_t - P_noise_in_bin_t
    return np.mean(P_signal_in_bin_t), np.mean(P_noise_in_bin_t)


def set_std_from_snr(
    waveform: NDArray[np.float64],
    mask: NDArray[np.uint8],
    parameters: Parameters,
    snr_db: float,
    ...
    freqs, _, Zxx = scipy_stft_complex_psd(
        waveform, parameters.audio.sampling_rate, parameters.stft
    )
    freqs, Zxx = frequency_band(
        freqs, Zxx, parameters.audio.min_freq, parameters.audio.max_freq
    ...
    signal_power, _ = compute_P_moy(Pxx, mask, global_noise_psd, df=df)
    ...
    return np.sqrt(
        signal_power * parameters.audio.sampling_rate / (snr * np.mean(B_in_t[valid]))
    )
